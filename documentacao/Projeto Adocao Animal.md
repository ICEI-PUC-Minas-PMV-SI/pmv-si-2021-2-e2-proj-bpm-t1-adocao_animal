...
+ **Relatório Adoção de Animais-(R3) ANIMAIS-tipoTabela-Cadastro de ANIMAIS-Ordenado por Nome**<p>
```SQL
SELECT Nome, Espécie, Sexo, Idade, Porte, Cor, Personalidade, Raça, Castrado, Data em que o animal foi resgatado, ONG, Veterinário.Nome, Adoção, Data da criação COUNT(Doenças Crônicas, Animal) AS Quantidade FROM (Adoção Animal - ANIMAL) ORDER BY Nome
```
+ **Relatório Adoção de Animais-(R3) ANIMAIS-tipoTabela-Relatório situação crítica dos ANIMAIS-Ordenado por Nome**<p>
```SQL
SELECT Nome, Espécie, Sexo, Idade, Porte, Cor, Personalidade, Raça, Castrado, Data em que o animal foi resgatado, ONG, Veterinário.Nome, Adoção, Data da criação COUNT(Doenças Crônicas, Animal) AS Quantidade FROM (Adoção Animal - ANIMAL) ORDER BY Nome
...
--------- | -------- | --------- | ------------------ | --------------- | ----------
Cadastro de Adotantes | Analisar se a base de dados de adotantes cadastrados atente a demanda de animais disponíveis para adoção e utilizar esse dados, para desenvolver estratégias de marketing do programa. | Percentual comparativo entre a quantidade de interessados em adotar um animal e a quantidade de animais disponíveis para adoção. | (cadastro_de_adotantes / cadastro_de_animais) * 100 | Processo de Cadastro de Adotantes e Pós-Adoção | Marketing |
Cadastro de Animais | Realizar uma analise referente ao cadastro de animais na base de dados em períodos pré-determinados e utilizar a informação para a avaliação de custos operacionais envolvendo alimentação, higiene e demais necessidades operacionais. | Percentual comparativo entre o cadastro de novos animais e a saída de animais por meio do processo de adoção. | (cadastro_de_animais / pos_adocao) * 100 | Processo de Cadastro de Animais e Pós Adoção | Operacional |
Adoções realizadas | Entender se o processo de adoção está atingindo uma porcentagem mínima pré-definida para o período observado e utilizar esses dados, para definir propostas de educação comunitária com relação ao compromisso de se ter um animal sob sua responsabilidade | Avaliar o sucesso e consolidação das adoções e desenvolver programas de educação comunitária. | (pos_adocao / adocao) * 100 | Processo de Pós-Adoção e Adoção | Educacional |
Saúde animal | Analisar proporcionalmente a saúde dos animais recém recolhidos e abrigados, buscando avaliar a necessidade de novas parcerias entre clínicas e ou veterinários e também utilizar a detecção de doenças transmissíveis, para desenvolver programas junto aos órgãos responsáveis pelo combate a zoonoses. | Analisar o percentual de animais que necessitam de tratamento/portadores de doenças transmissíveis. | (requer_tratamento / cadastro_de_animais) * 100 | Tratamentos e Processo de Cadastro de Animais | Ambiental |
Avaliação do Programa | Buscar avaliar o sucesso dos processos de adoção, realizando uma analise da proporção de adoções consolidadas, comparado com o retorno/devolução do animal. | Avaliar o programa como um todo e identificar se o mesmo está atendendo as expectativas. | (animais_devolvidos / adocoes_consolidadas) * 100 | Tabela de Processo de Pós-Adoção | Administrativo |
